...
// 👩‍🦰 பெண்கள் குழு (Contact number removed)
const womenTeam = [
        ...
        {team.map((member, index) => (
          <motion.div
            ...
            className="p-6 rounded-2xl shadow-lg border border-red-100 backdrop-blur-sm 
                       ...
                       flex flex-col items-center justify-between text-center min-h-[320px] w-[250px]"
          >
            <motion.img
              src={member.img}
              alt={member.name}
            ...
            <div className="flex flex-col justify-center items-center flex-grow">
              ...
              {showContact && member.contact && (
                <p className="text-[#b30000] font-medium flex items-center justify-center gap-1">
                  📞{" "}
                  <a
    ...
    >
      <div className="max-w-7xl mx-auto text-center space-y-24">
        <motion.h2
          ...
          className="text-4xl font-bold text-[#b30000] mb-12 drop-shadow-sm"
        >
          கழகத்தின் மாநில நிர்வாகிகள்
        </motion.h2>

        {renderTeam(stateTeam)}
        <motion.h2
          initial={{ opacity: 0, y: -30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          className="text-4xl font-bold text-[#800000] mt-20 mb-12"
        >
          மாநில நிர்வாகக்குழு உறுப்பினர்கள்
        </motion.h2>
        {renderTeam(districtTeam, true)}

        <motion.h2
          initial={{ opacity: 0, y: -30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          className="text-4xl font-bold text-[#b30047] mt-20 mb-12"
        >
          மாநில நிர்வாகிகள் (மகளிர் அணி)
        </motion.h2>
        {renderTeam(womenTeam, false, false)}

        <motion.h2
          initial={{ opacity: 0, y: -30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          className="text-4xl font-bold text-[#d63384] mt-20 mb-12"
        >
          மாநில நிர்வாகக்குழு உறுப்பினர்கள் (மகளிர் அணி)
        </motion.h2>
        {renderTeam(womenDistrictTeam, true, false)}
      </div>
    </section>
  );
}
