...
const galleryItems = [
  ...
  const handleSeeMore = () => {
    setVisibleCount((prev) => Math.min(prev + 4, galleryItems.length));
  };

  return (
  <section className="pt-16 pb-16 bg-gradient-to-br from-[#ffe5e5] via-[#fff0f0] to-[#ffcccc]">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto my-5 px-4">
          <h2 className="text-4xl font-bold mb-4">Success Gallery</h2>
        ...
        {/* Gallery Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-6 justify-items-center">
...
// ✅ GalleryCard with skeleton loader
const GalleryCard = ({ item, openModal }) => {
  const [loaded, setLoaded] = useState(false);

  return (
    <div
      className="bg-white rounded-lg overflow-hidden w-full max-w-xs cursor-pointer transform hover:scale-105 transition"
      onClick={() => openModal(item)}
    >
      {/* Skeleton while loading */}
      {!loaded && (
        <div className="w-full h-96 bg-gray-200 animate-pulse relative overflow-hidden">
      ...
      <img
        src={item.image}
        alt={item.title}
        className={`w-full h-96 object-cover transition-opacity duration-500 ${loaded ? "opacity-100" : "opacity-0"
          }`}
